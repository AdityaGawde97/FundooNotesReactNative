...
exports.getOnCellTap = proc(function (state, tapState, disabled, offset, scrollOffset, hasMoved, hoverTo, touchCellOffset, onGestureRelease, touchOffset) {
    return block([
        cond(and(neq(state, tapState), not(disabled)), [
            ...
            cond(eq(state, react_native_gesture_handler_1.State.END), onGestureRelease)
        ])
    ]);
});
